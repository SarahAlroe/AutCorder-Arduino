#pragma once

#include "Arduino.h"
#include "ESP_I2S.h"

#define MIC_CLK 47
#define MIC_DATA 48

class Dictaphone {
  public:
    Dictaphone();

    bool begin(); // Initialize I2S
    void warmup(); // Read and discard some samples (should have short execution time)
    void beginRecording(); // Begin making a recording
    void continueRecording(); // Keep this going with little to no delay while recording
    void processRecording(float limiterFactor);
    bool saveRecording(String filePrefix); // Save it out to an SD card
    uint16_t getSecondsRecorded();

  private:
    I2SClass i2s;
    // Audio recording settings
    const uint16_t RECORD_MAX_S = 120;          // 120 seconds as a low-ball. PSRAM 4 or 8 MB. Not in use when not taking pictures. at 16k samples at 16bit (2bytes), thats 125 or 250 seconds, 2 or 4 minutes of recording.
    const uint16_t RECORD_SAMPLE_RATE = 16000;  // 16khz
    const size_t RECORD_MAX_SIZE = sizeof(uint8_t) * ((RECORD_SAMPLE_RATE * (I2S_DATA_BIT_WIDTH_16BIT / 8)) * I2S_SLOT_MODE_MONO) * RECORD_MAX_S;
    uint8_t *wavBuffer;
    size_t recordingLength;
    size_t millisecondLength;
};
